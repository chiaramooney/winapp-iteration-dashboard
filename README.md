...
```
GitHub Actions (daily)  ──────────►  build-data.js  ──►  docs/data.json (committed)
                                                             │
...
- Every day at **06:00 UTC** the workflow refreshes the current and previous
  calendar months and merges them into `docs/data.json`. This automatically
  repairs gaps from missed runs while preserving older history.
...
4. **Seed older data** (optional): run the workflow manually with a range to
   backfill, e.g. `2026-01 2026-06`. Scheduled runs maintain the latest two
   calendar months.
